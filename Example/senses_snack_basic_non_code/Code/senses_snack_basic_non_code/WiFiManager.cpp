...
void WiFiManager::setupConfigPortal() {
  ...
  /* Setup web pages: root, wifi config s, SO captive portal detectors and not found. */
...
void WiFiManager::handleRoot() {
  ...
  if (captivePortal()) { // If caprive portal redirect instead of displaying the .
    return;
  }
...
void WiFiManager::handleWifiSave() {
  ...
  for (int i = 0; i < _paramsCount; i++) {
    ...
    DEBUG_WM(value);
      ...
      if(i==0){
      String senses_id = value;
      writeString(6, senses_id);
      Serial.print("WF_SENSES_ID"); Serial.println(senses_id);
    }

      else if(i==1){
      String senses_key = value;
      writeString(20, senses_key);
      Serial.print("WF_SENSES_KEY"); Serial.println(senses_key);
    }
      ...
      if(i==0){
      String senses_id = value;
      writeStringESP32(6, senses_id);
      Serial.print("WF_SENSES_ID"); Serial.println(senses_id);
    }

      else if(i==1){
      String senses_key = value;
      writeStringESP32(20, senses_key);
      Serial.print("WF_SENSES_KEY"); Serial.println(senses_key);
    }
...
#if defined(ESP8266) 
void WiFiManager::writeString(char add,String data)
{
...
#else
void WiFiManager::writeStringESP32(int add,String data)
{
 
  EEPROM.writeString(add,data); 
  EEPROM.commit();
}
 
 
String WiFiManager::read_StringESP32(int add)
{
  String data = EEPROM.readString(add);
  return String(data);
}
#endif 
